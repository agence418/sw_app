...
export const VoteView = () => {
  ...
  const fetchIdeas = async () => {
    ...
    try {
      ...
      console.error('Erreur lors de la récupération des idées:', _error);
    } finally {
      setLoading(false);
    }
  };
  ...
  //         }
  //     } catch (_error) {
  //         console.error('Erreur lors de la vérification des votes:', _error);
  //     } finally {
  ...
  const toggleIdeaSelection = async (projectName: string) => {
    if (!session?.user?.id) {
      setSubmitMessage({ type: '_error', text: 'Vous devez être connecté pour voter' });
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmitMessage({ type: '_error', text: "Erreur lors de l'enregistrement du vote" });
      }
    } catch (_error) {
      console.error('Erreur lors de la soumission des votes:', _error);
      setSubmitMessage({ type: '_error', text: "Erreur lors de l'enregistrement des votes" });
    } finally {
  ...
  return (
    <div className="bg-white dark:bg-black rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-800">
      ...
      {checkingVotes ? (
        <div className="text-center py-8 text-gray-500">Vérification de vos votes...</div>
      ) : projects.length === 0 ? (
        <div className="text-center py-8 text-gray-500">Aucun projet disponible pour le moment</div>
      ) : (
        <>
