...
export const IdeaCreationView = () => {
  ...
  const fetchParticipants = async () => {
    try {
      ...
      console.error('Erreur lors de la récupération des participants:', _error);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      newIdea.participantName =
        participants.find((participant) => participant.id.toString() === newIdea.participantId)
          ?.name || '';
      ...
      console.error('Erreur lors de la création du projet:', _error);
  ...
  return (
    <>
      <div className="bg-white dark:bg-black rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-800">
        ...
        {successMessage && (
          <div className="mb-4 p-3 bg-green-50 dark:bg-green-900 text-green-600 dark:text-white rounded-lg text-sm">
            {successMessage}
          </div>
        )}
              ...
              {ideas.map((idea) => (
                <div key={idea.id} className="p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                  ...
                  <div className="text-sm text-gray-600 dark:text-gray-400">
                    {idea.description ?? 'Tu pe'}
                  </div>
                  <div className="text-xs text-gray-500 mt-1">Par: {idea.participantName}</div>
                </div>
              ))}
            </div>
          </div>
