  ...
  const generateQRCode = async () => {
    try {
      ...
      const res = await fetch(
        `/api/auth/get-reset-password-link?email=${encodeURIComponent(user.email)}`
      );
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data._error || 'Erreur lors de la génération du lien');
      }
      ...
      <div className="bg-white dark:bg-black rounded-lg p-6 w-11/12 max-w-md max-h-[100vh] overflow-auto">
        ...
        {user && (
          <>
            ...
            <p className="text-xs font-semibold mb-8 text-center text-gray-600 dark:text-gray-400 w-full">
              {user.email}
            </p>
          </>
        )}
        <p className="text-sm text-gray-600 dark:text-gray-200 mb-4">
          Scannez ce QR code pour rediriger l'utilisateur vers la page de réinitialisation du mot de
          passe.
        </p>
        {onModalQRCode ? (
