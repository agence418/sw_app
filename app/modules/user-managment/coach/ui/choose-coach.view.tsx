...
export const ChooseCoachView = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('Erreur lors du chargement des données:', _error);
      setMessage({ type: '_error', text: 'Erreur lors du chargement des données' });
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      if (isChecked) {
        ...
        if (coachPreferences.length >= 3) {
          setMessage({ type: '_error', text: 'Vous ne pouvez sélectionner que 3 coachs maximum' });
          return;
        }
        ...
        if (response.ok) {
          ...
          setMessage({ type: '_error', text: _error._error || "Erreur lors de l'ajout" });
        }
      } else {
        ...
        if (response.ok) {
          ...
          setMessage({ type: '_error', text: 'Erreur lors de la suppression' });
        }
      }
    } catch (_error) {
      console.error('Erreur:', _error);
      setMessage({ type: '_error', text: 'Erreur lors de la modification' });
    }
          ...
          return (
            ...
            >
              ...
              <div className="flex-1">
                ...
                {coach.expertise && <div className="text-sm text-gray-600">{coach.expertise}</div>}
              </div>
              {isSelected && (
