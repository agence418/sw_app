...
import {
    getEventFromStageID,
    getNextEventFromStageID, getPreviousEventFromStageID
    ...
    try {
        ...
        useCurrentStatus.setState({
            status: {
                ...
                nextEvent: getNextEventFromStageID((status.currentEvent?.step ?? 0), config.event_start_date),
                previousEvent: getPreviousEventFromStageID(status.currentEvent?.step ?? 0, config.event_start_date),
