...
export const ConfigView = () => {
  const { loading, _error, success, config } = useConfig();

  const saveConfig = async (updatedConfig: AppConfig) => {
    try {
      useConfig.setState({ loading: true, _error: undefined, success: undefined });
      ...
      useConfig.setState({ _error: 'Erreur lors de la sauvegarde' });
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      {_error && (
        <div className="p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">{_error}</div>
      )}
      ...
      <div className="bg-white dark:bg-black rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 p-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
            Jour de fin :{' '}
            {new Date(
              new Date(config.event_start_date).getTime() + 2 * 24 * 60 * 60 * 1000
