...
export const ForgotPasswordView: React.FC<ForgotPasswordViewProps> = ({ onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.trim()) {
      setMessage({ type: '_error', text: 'Veuillez saisir votre adresse email' });
      return;
    }

    if (!email.includes('@')) {
      setMessage({ type: '_error', text: 'Veuillez saisir une adresse email valide' });
      return;
    }
    ...
    <div className="flex items-center justify-center bg-white dark:bg-black h-screen">
      <div className="p-8 w-full h-screen items-center justify-center flex flex-col">
        <div className="text-center mb-8">
          ...
          <p className="text-sm">
            Saisissez votre adresse email et nous vous enverrons un lien pour créer/réinitialiser
            votre mot de passe.
          </p>
        </div>
