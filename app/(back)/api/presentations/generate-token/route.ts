...
export async function POST(_request: NextRequest) {
  ...
  if (session?.user?.role !== 'participant') {
    return NextResponse.json({ _error: 'Accès réservé aux participants' }, { status: 403 });
  }
  ...
  try {
    ...
    if (!team) {
      return NextResponse.json(
        { _error: "Vous devez être leader d'une équipe pour envoyer une présentation" },
        { status: 400 }
      );
    }
    ...
    const baseUrl =
      process.env.NEXTAUTH_URL || `${_request.nextUrl.protocol}//${_request.nextUrl.host}`;
    ...
    console.error('Erreur lors de la génération du token:', _error);
    return NextResponse.json({ _error: 'Erreur lors de la génération du lien' }, { status: 500 });
  }
}
