...
export async function POST(_request: NextRequest) {
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return NextResponse.json({ _error: 'Token manquant' }, { status: 401 });
    }
    ...
    try {
      ...
      return NextResponse.json({ _error: 'Token invalide ou expiré' }, { status: 401 });
    }

    // Vérifier que le token est bien pour l'upload de présentation
    if (decoded.type !== 'presentation-upload') {
      return NextResponse.json({ _error: 'Token non autorisé pour cette action' }, { status: 403 });
    }

    // Vérifier que le webhook n8n est configuré
    if (!N8N_WEBHOOK_URL) {
      console.error("N8N_PRESENTATION_WEBHOOK_URL n'est pas configuré");
      return NextResponse.json({ _error: "Service d'upload non configuré" }, { status: 500 });
    }
    ...
    if (!file) {
      return NextResponse.json({ _error: 'Aucun fichier fourni' }, { status: 400 });
    }
    ...
    if (!fileName.endsWith('.ppt') && !fileName.endsWith('.pptx') && !fileName.endsWith('.pdf')) {
      return NextResponse.json(
        { _error: 'Format de fichier non accepté. Utilisez .ppt, .pptx ou .pdf' },
        { status: 400 }
      );
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { _error: "Erreur lors de l'envoi vers le serveur" },
        { status: 500 }
      );
    }
    ...
    console.error("Erreur lors de l'upload:", _error);
    return NextResponse.json({ _error: "Erreur lors de l'envoi du fichier" }, { status: 500 });
  }
}
