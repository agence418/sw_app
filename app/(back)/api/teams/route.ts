...
export async function POST(_request: Request) {
  try {
    ...
    if (!name || !leader_id) {
      return NextResponse.json({ _error: 'Tous les champs sont requis' }, { status: 400 });
    }
    ...
    console.error("Erreur lors de la création de l'équipe:", _error);
    return NextResponse.json({ _error: "Erreur lors de la création de l'équipe" }, { status: 500 });
  }
}
