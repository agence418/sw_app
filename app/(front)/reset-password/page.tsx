...
function ResetPasswordComponent() {
  ...
  const [message, setMessage] = useState<{ type: 'success' | '_error'; text: string } | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (passwordError) {
      setMessage({ type: '_error', text: passwordError });
      return;
    }

    if (password !== confirmPassword) {
      setMessage({ type: '_error', text: 'Les mots de passe ne correspondent pas' });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: '_error', text: data._error || 'Une erreur est survenue' });
      }
    } catch (_error) {
      setMessage({ type: '_error', text: 'Erreur de connexion. Veuillez réessayer.' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="p-8 w-full h-screen items-center justify-center flex flex-col">
          <div className="text-center">
            ...
            <p className="text-sm mb-6">
              Ce lien de réinitialisation est invalide ou a expiré. Veuillez faire une nouvelle
              demande.
            </p>
            <button
              onClick={() => router.push('/')}
    ...
    <div className="flex items-center justify-center bg-white dark:bg-black h-screen">
      <div className="p-8 w-full h-screen items-center justify-center flex flex-col">
        <div className="text-center mb-8">
          ...
          <p className="text-sm">Choisissez un mot de passe sécurisé pour votre compte.</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="mt-1 block w-full px-3 py-2 pr-10 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-600 focus:border-green-600 dark:text-gray-800"
                required
              />
              <button
                type="button"
