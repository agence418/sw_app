  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData._error || "Erreur lors de l'envoi");
      }
      ...
      setError(_err.message || "Erreur lors de l'envoi du fichier");
    } finally {
      setIsUploading(false);
    }
  };

  if (_error && !isValidToken) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
        <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-xl p-8 shadow-lg text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Ce lien n'est plus valide ou a expiré. Veuillez générer un nouveau lien depuis votre
            application mobile.
          </p>
        </div>
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
        <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-xl p-8 shadow-lg text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Votre présentation a été envoyée avec succès pour l'équipe{' '}
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
      <div className="max-w-2xl w-full bg-white dark:bg-gray-800 rounded-xl p-8 shadow-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Équipe:{' '}
            <strong className="text-blue-600 dark:text-blue-400">{teamInfo?.teamName}</strong>
          </p>
        ...
        >
          <Upload
            className={`w-20 h-20 mx-auto mb-6 ${isDragging ? 'text-blue-500 animate-bounce' : 'text-gray-400'}`}
          />
          <input
            type="file"
            accept=".ppt,.pptx,.pdf"
        ...
        {_error && (
          <div className="mt-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
            <p className="text-red-600 dark:text-red-400 text-center">{_error}</p>
          </div>
        )}
