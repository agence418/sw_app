...
export const StartupWeekendCoachApp = () => {
  ...
  if (status?.currentEvent?.step > 19) {
    return (
      <div className="min-h-screen text-white bg-green-600 flex items-center justify-center p-4">
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="p-4">
        {/* Page d'accueil */}
        {status.votesAllowed && config.who_can_vote.includes('coach') ? (
          <VoteView />
          ...
          <div className="space-y-6">
            <div className="rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-800">
              <h2
                className={`text-lg font-semibold flex items-center ${config.who_can_vote.includes('visitor') ? 'mb-4' : ''}`}
              >
              ...
              {config.who_can_vote.includes('coach') ? (
                <div className="text-gray-500">
                  Vous pourrez bientôt voter pour vos projets favoris
                </div>
              ) : (
                <div className="text-gray-500">
                  Vous pourrez bientôt retrouver la liste des équipes à coacher
                </div>
              )}
            </div>
          </div>
